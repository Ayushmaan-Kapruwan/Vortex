...
// main recommendation func for integration. -yb
static void showRecommendations() {
  system("python analytics/recommend.py");

  std::ifstream file("analytics/recommendations.json");

  if (!file.is_open()) {
    cout << "Failed to open recommendations file.\n";
    return;
  }
...
static void run_games_menu(std::vector<UnifiedGame>& games, const std::string& menu_title) {
  ...
  std::sort(games.begin(), games.end(),
    [](const UnifiedGame& a, const UnifiedGame& b) {
  ...
  while (true) {
    ...
    for (size_t i = 0; i < games.size(); ++i) {
      ...
      if (menu_title == "All Games") {
        if (games[i].source == GameSource::Steam)
          cout << " (Steam)";
        else
          cout << " (Local)";
      }
      ...
      std::string key =
        (games[i].igdb_id != 0)
          ...
          : (games[i].source == GameSource::Steam
    ...
    while (true) {
      ...
      std::string key =
        (selected.igdb_id != 0)
          ...
          : (selected.source == GameSource::Steam
      ...
      if (action == 1) {
        ...
        if (selected.source == GameSource::Steam) {
          ...
          if (end_time > start_time) {
            ...
            int active_seconds = static_cast<int>(duration * 0.9);
            int idle_seconds = static_cast<int>(duration * 0.1);

              std::string command =
                "python analytics/ingest_session.py "
                "user1 \"" +
              selected.name + "\" " +
              std::to_string(start_time) + " " +
              std::to_string(end_time) + " " +
              std::to_string(active_seconds) + " " +
              std::to_string(idle_seconds);

system(command.c_str());
system("python analytics/train.py"); //model training after sessions. -yb
          }
        } else {
        ...
        toggle_game_preference(selected.name, 1.0);
      } else if (action == 3) {
        toggle_game_preference(selected.name, -1.0);
      } else if (action == 4) {
        std::error_code ec;
...
int main() {
  ...
  for (const auto& g : localGamesInit)
    allGameNames.push_back(g.name);

  for (const auto& g : steamGamesInit)
    allGameNames.push_back(g.name);
...
// changed from if else block to switch-case -yb
    switch (mode) {
      case 0:
        cout << "Cya chap\n";
        return 0;
